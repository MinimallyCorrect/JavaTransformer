...
import static dev.minco.javatransformer.api.code.IntermediateValue.LocationType.LOCAL;
import static dev.minco.javatransformer.api.code.IntermediateValue.LocationType.STACK;
...
import dev.minco.javatransformer.internal.asm.AsmInstructions;
import dev.minco.javatransformer.internal.asm.CombinedValue;
import dev.minco.javatransformer.internal.asm.DebugPrinter;
import dev.minco.javatransformer.internal.util.Cloner;
...
class AsmCodeFragmentGenerator implements Opcodes {
	...
	abstract static class AsmCodeFragment implements CodeFragment {
		...
		public void insert(@NonNull CodeFragment fragmentOfAnyType, @NonNull InsertionPosition position, @NonNull InsertionOptions insertionOptions) {
			if (CodeFragmentUtil.validateInsert(this, fragmentOfAnyType, position, insertionOptions)) {
				return;
			}
			...
			if (options.convertReturnCallToReturnInstruction) {
				...
				while (true) {
					...
					if (opcode == INVOKESTATIC || current instanceof MethodInsnNode) {
						...
						if (JVMUtil.classNameToSlashName(dev.minco.javatransformer.api.code.RETURN.class).equals(methodInsnNode.owner)) {
